...
// ONE profile, so that collapses to: hard = allergies + hard_excludes; soft = soft_excludes; proteins/
// weights = the profile's. No member-name starring, no strong-soft consensus (both need >=2 members -> v2).
// Built by PreferencesService.ComputeEffectivePreferences(); all sets/keys arrive lowercased+trimmed.
// Forward-compatible: the v2 master member fills the same fields.
