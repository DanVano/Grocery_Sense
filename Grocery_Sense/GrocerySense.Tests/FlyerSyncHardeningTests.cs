...
public sealed class FlyerSyncHardeningTests : IDisposable
{
    private readonly string _dir = Path.Combine(Path.GetTempPath(), $"gs_synchard_{Guid.NewGuid():N}");
    private readonly SqliteConnectionFactory _factory;
    private readonly ConfigStore _config;
    private readonly string _metaPath;

    public FlyerSyncHardeningTests()
    {
    ...
    private sealed class FuncProvider(Func<string, IReadOnlyList<Dictionary<string, object?>>> fn) : IFlyerProvider
    {
            ...
            string storeName, string postalCode, CancellationToken ct = default)
        {
            ...
            return Task.FromResult(fn(storeName));
        }
    }
